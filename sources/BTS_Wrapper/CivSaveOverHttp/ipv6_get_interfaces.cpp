...
char* __cdecl get_ip6_for_ip4(const char *_ip4, bool reverse_lookup)
{
  ...
  do {
    ...
    if (pAddresses == NULL) {
      LOGPRINT("Memory allocation failed for IP_ADAPTER_ADDRESSES struct");
      return ret;
    }
  ...
  if (dwRetVal == NO_ERROR) {
    ...
    while (pCurrAddresses) {
#if 0
      LOGPRINT("\n\n\tLength of the IP_ADAPTER_ADDRESS struct: " << pCurrAddresses->Length);
      LOGPRINT("\tIfIndex (IPv4 interface): " << pCurrAddresses->IfIndex);
      LOGPRINT("\tAdapter name: " << pCurrAddresses->AdapterName);

      pUnicast = pCurrAddresses->FirstUnicastAddress;
      if (pUnicast != NULL) {
        for (i = 0; pUnicast != NULL; i++)
          pUnicast = pUnicast->Next;
        LOGPRINT("\tNumber of Unicast Addresses: " << i);
      } else {
        LOGPRINT("\tNo Unicast Addresses");
      }
      ...
      if (pAnycast) {
        ...
        LOGPRINT("\tNumber of Anycast Addresses: " << i);
      } else {
        LOGPRINT("\tNo Anycast Addresses");
      }
      ...
      if (pMulticast) {
        ...
        LOGPRINT("\tNumber of Multicast Addresses: " << i);
      } else {
        LOGPRINT("\tNo Multicast Addresses");
      }
      ...
      if (pDnServer) {
        ...
        LOGPRINT("\tNumber of DNS Server Addresses: " << i);
      } else {
        LOGPRINT("\tNo DNS Server Addresses");
      }

      LOGPRINT("\tDNS Suffix: " << pCurrAddresses->DnsSuffix);
      LOGPRINT("\tDescription: " << pCurrAddresses->Description);
      LOGPRINT("\tFriendly name: " << pCurrAddresses->FriendlyName);

      if (pCurrAddresses->PhysicalAddressLength != 0) {
        LOGPRINT0("\tPhysical address: ");
      ...
      LOGPRINT("\tFlags: " << pCurrAddresses->Flags);
      LOGPRINT("\tMtu: " << pCurrAddresses->Mtu);
      LOGPRINT("\tIfType: " << pCurrAddresses->IfType);
      LOGPRINT("\tOperStatus: " << pCurrAddresses->OperStatus);
      LOGPRINT("\tIpv6IfIndex (IPv6 interface): " << pCurrAddresses->Ipv6IfIndex);
      LOGPRINT0("\tZoneIndices (hex): ");
      for (i = 0; i < 16; i++) {
        LOGPRINT0( std::hex << pCurrAddresses->ZoneIndices[i]);
      }
      LOGPRINT("");

      LOGPRINT("\tTransmit link speed: " << pCurrAddresses->TransmitLinkSpeed);
      LOGPRINT("\tReceive link speed: " << pCurrAddresses->ReceiveLinkSpeed);
...
#if PREFER_TEMP_IPv6
            LOGPRINT("Found temporary address");
#else
            LOGPRINT("Found global static address");
#endif
    ...
    if (pBestUnicast) {
      ...
      _tmp 
        ...
        << ":" << std::hex << (int)((bar->sin6_addr.u.Byte[14] << 8) | bar->sin6_addr.u.Byte[15])
        << "]";
      LOGPRINT("BEST IPv6 FOUND: " << _tmp.str());
      ret = _strdup(_tmp.str().c_str());
    }


  } else {
      LOGPRINT("Call to GetAdaptersAddresses failed with error: " << dwRetVal);
      if (dwRetVal == ERROR_NO_DATA) {
        LOGPRINT("\tNo addresses were found for the requested parameters");
      } else {
